        ...
        (err, table) => {
          ...
          if (table) {
            ...
            db.all('SELECT * FROM produse', (err, rows) => {
              if (err) throw err;
              res.render('demo_page', { products: rows, authenticated: authenticated, admin: admin,username: username });
            });
          } else {
            res.render('demo_page', { products: [], authenticated: authenticated, admin: admin,username: username });
          }
        }
      );
    });
...
app.get('/inserare-bd', (req, res) => {
  db.serialize(() => {
    ...
    db = new sqlite3.Database('cumparaturi.db', (err) => {
      ...
      const insertQuery = 'REPLACE INTO produse (id, nume, pret) VALUES (?, ?, ?);';
