...
void multiply_GF128(uint8_t* a, const uint8_t* b) {
	...
	for (int i = 0; i < 128; ++i) {
		...
		// Shift a_copy to the right
		uint8_t msb = a_copy[15] & 1; // Save the MSB before shifting

		shift_right(a_copy);
		// If the MSB was 1, XOR with the reduction polynomial
		if (msb) {
			a_copy[0] ^= 0xE1; // This is the reduction polynomial for GF(2^128) in GCM
		}
	}
...
{
	std::vector<std::vector<uint8_t>> X;
	X.reserve(m + n + 2); // Pre-allocate memory
	long long int lenA;
	long long int lenC;
	if (m == 0)
	{
...
vector<uint8_t> incr(const vector<uint8_t> ina) {
	...
	// Increment the rightmost 32 bits of the input vector
	uint32_t value = 0;
...
void GCMDecrypt(string& inputFile, string& keyFile, string& AAD, string& tag, string& Iv)
{

	vector<uint8_t> iv; /* An initialization vector IV, that can have any number of bits between 1 and 2^64. For a fixed
value of the key, each IV value must be distinct, but need not have equal lengths. 96 - bit
	...
	C.reserve(estimatedLines);

	size_t rowSize = 16;
	for (size_t i = 0; i < inputFile.size(); ++i) {
		if (C.empty() || C.back().size() == rowSize) {
			C.push_back(vector<uint8_t>());
			n++;
		}
		C.back().push_back(inputFile[i]);
	}
	if (!C.empty() && C.back().empty()) {
		C.pop_back();
		n--;
	}
	u = C.empty() ? 0 : C.back().size() * 8;
	...
	vector<uint8_t> T_enc;//An authentication tag T_enc, whose length can be any value between 0 and 128. The length of the tag is denoted as t.
	int t = 128;//length of T_enc
	vector<uint8_t*> KY;
	...
	for (int i = 0; i < t / 8; ++i) {
		T_enc.push_back((X.back()[i]) ^ (KY[0][i]));
	}




	for (int i = 0; i < T_enc.size(); ++i)
	{
		if (T_enc[i] != (uint8_t)tag[i]) {

			std::cerr << "Error: Tag doesn't match. Message integrity can't be validated." << std::endl;
			return;
		}
	}
	...
	for (int j = 0; j < u / 8; ++j)
	{
		P[n - 1].push_back(C[n - 1][j] ^ KY[n][j]);
	}
...
int main(int argc, char* argv[])
{
	...
	std::string tagBase64;
	std::getline(std::cin, tagBase64);
	std::string ivBase64;
	...
	std::string tag = decode_base64(tagBase64);
	std::string Iv = decode_base64(ivBase64);

	GCMDecrypt(data, key, aed, tag, Iv);
